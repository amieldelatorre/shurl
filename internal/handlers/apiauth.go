...
func (h *ApiAuthHandler) Login(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		EncodeResponse[LoginResponse](h.Logger, r.Context(), w, errorCode, LoginResponse{Errors: []string{message}})
	...
	if err != nil {
		...
		if errors.As(err, &vError) {
			EncodeResponse[LoginResponse](h.Logger, r.Context(), w, http.StatusBadRequest, LoginResponse{Errors: EncodeValidationError(vError)})
			return
		}
	...
	if user == nil {
		...
		EncodeResponse[LoginResponse](h.Logger, r.Context(), w, http.StatusUnauthorized, LoginResponse{Errors: []string{invalidCredentialsMessage}})
		// TODO: Add IP address
