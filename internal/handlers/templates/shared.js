...
export async function createShortUrl(event) {
    event.preventDefault();
    const submittingButton = event.submitter;
    changeButtonToLoading(submittingButton);

    const destinationUrlInput = document.getElementById("url-input");
    const destinationUrl = destinationUrlInput.value;

    const urlTtlInput = document.getElementById("url-ttl");
    const urlTtl = Number(urlTtlInput.value);

    const urlTtlUnitInput = document.getElementById("ttl-units");
    const ttlUnit = Number(urlTtlUnitInput.value);
